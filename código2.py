...
import osmnx as ox
import osmnx as _oxmod  # alias used inside compute_route
...
# Overpass endpoint — change if the default is unreachable in your network
...
def load_graph(lat, lon, radius=900):
    ...
    # Project to UTM so nearest_nodes uses fast BallTree without scikit-learn dep
...
def _latlon_to_proj(G, lat, lon):
    """Convert WGS84 lat/lon to the graph projected CRS (UTM)."""
    crs = G.graph.get("crs", "EPSG:4326")
    transformer = pyproj.Transformer.from_crs("EPSG:4326", str(crs), always_xy=True)
    x, y = transformer.transform(lon, lat)
    return x, y

def _node_latlon(G, n):
    """Return (lat, lon) of a projected graph node back in WGS84."""
    crs = G.graph.get("crs", "EPSG:4326")
    transformer = pyproj.Transformer.from_crs(str(crs), "EPSG:4326", always_xy=True)
    lon, lat = transformer.transform(G.nodes[n]["x"], G.nodes[n]["y"])
    return lat, lon

def compute_route(G, origin_ll, dest_ll, profile):
    """Find shortest accessible path between two lat/lon pairs."""
    try:
        ox_orig, oy_orig = _latlon_to_proj(G, origin_ll[0], origin_ll[1])
        ox_dest, oy_dest = _latlon_to_proj(G, dest_ll[0], dest_ll[1])
        orig_node = _oxmod.distance.nearest_nodes(G, X=ox_orig, Y=oy_orig)
        dest_node = _oxmod.distance.nearest_nodes(G, X=ox_dest, Y=oy_dest)
        ...
        # Extract route geometry and stats (convert projected nodes back to lat/lon)
        coords = [_node_latlon(G, n) for n in path]
        total_dist = sum(
